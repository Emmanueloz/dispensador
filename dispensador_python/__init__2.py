...
def enviar_tiempo_comida():
    ...
    if unidad and tiempo:
        ...
        messagebox.showwarning(
            "Advertencia", "Selecciona unidad y tiempo antes de enviar.")
...
def monitorear_estado_servo(etiqueta_estado):
    while True:
        try:
            ...
            estado_actual = mapa_posiciones.get(
                posicion_servo, "...Esperando..")
            ...
            etiqueta_estado.config(text=f"Error de comunicación serial: {
                                   str(serial_error)}")
        except Exception as error:
            etiqueta_estado.config(
                text=f"Error inesperado al obtener la posición del servo de alimento: {str(error)}")
...
def monitorear_estado_servo_agua(etiqueta_estado):
    while True:
        try:
            ...
            estado_actual_agua = mapa_posiciones_agua.get(
                posicion_servo_agua, "...Esperando..")
            ...
            etiqueta_estado.config(
                text=f"Error inesperado al obtener la posición del servo de agua: {str(error)}")
...
lbl_estado_comidaIni = Label(
    inicio, fg="red", font=("Courier New", 10, "bold"))
...
lbl_dispensando = Label(inicio, text="", fg="green",
                        font=("Courier New", 14, "bold"))
lbl_dispensando.place(x=450, y=390)

lbl_dispensandoAgua = Label(
    inicio, text="", fg="green", font=("Courier New", 14, "bold"))
lbl_dispensandoAgua.place(x=30, y=390)


lbl_contene_comida = Label(inicio, text="", fg="red",
                           font=("Courier New", 14, "bold"))
...
tiempo_agua = Scale(tiempo,  from_=0, to=60, orient="vertical", tickinterval=30,
                    length=400, variable=tiempo_agua_var).grid(row=1, column=0, rowspan=4, padx=5, pady=5)
tiempo_comida_var = IntVar()  # Variable para almacenar el valor de la escala
tiempo_comida = Scale(tiempo, from_=0, to=60, orient="vertical", tickinterval=30, length=400,
                      variable=tiempo_comida_var).grid(row=1, column=5, rowspan=4, padx=5, pady=5)
...
hilo_monitoreo_agua = threading.Thread(
    target=monitorear_estado_servo_agua, args=(lbl_estado_agua,))
hilo_monitoreo_agua.start()
hilo_monitoreo_contenedor_alimento = threading.Thread(
    target=monitorear_estado_contenedor_alimento, args=(lbl_contene_comida,))
...
comida = ttk.Treeview(consulta, height=10,
